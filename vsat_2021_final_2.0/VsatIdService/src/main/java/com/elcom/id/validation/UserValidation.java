...
//        }
    ...
    public String validateUpdateUserByUserLogin(User user) {
        if (user == null || user.getUuid() == null) {
            return "payLoad không hợp lệ";
        } else if (StringUtil.isNullOrEmpty(user.getEmail()) && StringUtil.isNullOrEmpty(user.getMobile())) {
            getMessageDes().add("email hoặc mobile không được để trống ");
    ...
    public String validateUpdatePassword(AuthorizationResponseDTO dto, String curentPassword,
        ...
        if (StringUtil.isNullOrEmpty(curentPassword)) {
            ...
            getMessageDes().add("Độ dài mật khẩu phải lớn hơn 6 và nhỏ hơn 12 ");
        } else if (StringUtil.isNullOrEmpty(newPassword) || StringUtil.isNullOrEmpty(rePassword)) {
            getMessageDes().add("Mật khẩu mới không được để trống");
        } else if (!newPassword.equals(rePassword)) {
            getMessageDes().add("Mật khẩu mới và mật khẩu nhập lại không trùng nhau");
        }

        return !isValid() ? this.buildValidationMessage() : null;
    }

    public String validateResetPassword(AuthorizationResponseDTO dto,
            String newPassword, String rePassword) throws ValidationException {
        ...
        if (StringUtil.isNullOrEmpty(newPassword) || StringUtil.isNullOrEmpty(rePassword)) {
            ...
            getMessageDes().add("Mật khẩu và mật khẩu nhập lại không trùng nhau");
        }
