...
import com.elcom.media.enums.PartitionType;
...
public class DateUtil {
    ...
    public static boolean isValidTimeByFormat(String value, String format) {
        Date date = null;
        try {
    ...
    public static List<String> getListPartitionBetweenTwoDates(Date startDate, Date endDate, int partitionType) {
        try {
            if( partitionType == PartitionType.PARTITION_TYPE_MONTH.toVal() ) {
                List<String> listPartition=new ArrayList<>();
                ...
                while (_curDate.getTime()<=endDate.getTime()){
                    ...
                    String _curDay=(_curDate.getDate())<10?"0"+String.valueOf(_curDate.getDate()):String.valueOf(_curDate.getDate());

                    listPartition.add(String.valueOf(_curDate.getYear()+1900)+String.valueOf(_curMonth)+String.valueOf(_curDay));
    ...
    public static Date getDateByHourAgo(Date currentDate, int prev) {
        try {
            Calendar c = Calendar.getInstance();
            c.setTime(currentDate);
            c.add(Calendar.HOUR, -prev);
            return c.getTime();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static int getPartitionByDate(Date _curDate) {
        try {
            int year=_curDate.getYear()+1900;
            String month=(_curDate.getMonth()+1)<10?"0"+String.valueOf(_curDate.getMonth()+1):String.valueOf(_curDate.getMonth()+1);
            String date=(_curDate.getDate())<10?"0"+String.valueOf(_curDate.getDate()):String.valueOf(_curDate.getDate());

            String sPartition=year+month+date;

            return Integer.parseInt(sPartition);
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
