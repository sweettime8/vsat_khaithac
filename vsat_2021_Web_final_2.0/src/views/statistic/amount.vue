<template>
  <div class="app-container">
    <h2 class="no-margin-top">Thống kê số bản tin vị trí theo chiều</h2>
    ...
    <el-row>
      <el-col :span="15">
        ...
        >
          ...
          <el-table-column label="Số bản tin FL" width="100">
              <template slot-scope="scope">
                  <span>{{ scope.row.totalFL  }}</span>
              </template>
          </el-table-column>

          <el-table-column label="Số bản tin RL" width="100">
              <template slot-scope="scope">
                  <span>{{ scope.row.totalRL  }}</span>
              </template>
          </el-table-column>

          <el-table-column label="Số bản tin khác" width="100">
              <template slot-scope="scope">
                  <span>{{ scope.row.totalUndef  }}</span>
              </template>
          </el-table-column>

          <el-table-column label="Tổng số bản tin" width="100"> 
              <template slot-scope="scope">
...
const amountflrlResource = new Resource("statistic/amount");
const amountChartistResource = new Resource("statistic/amountChartist");
...
//
  ...
  methods: {
    ...
    async getList() {
      ...
      const { data } = await amountChartistResource.listByPost(this.query);
      ...
      if(this.list_ApiPieData.size >0){
        ...
        for (const item of this.list_ApiPieData) {
            ...
            let list_pieChartDataMono= {
                      pieChartData: {
                            labels: ['Dung lượng FL : ' + item[1].totalFL + " ("+ dataset_data[0] +"%)", 'Dung lượng RL : ' + item[1].totalRL + "("+dataset_data[1] + "%)", 'Dung lượng khác : ' + item[1].totalUndef + "("+dataset_data[2]+"%)"],
                              datasets: [{
                                label: '',
